...
- [x] **A005** — Archive and evaluate the current CircleStone Anima license **and inherited NVIDIA Cosmos/Open Model terms** before distributing packs.
...
- [x] **D006** — ModelStore download/verify/ready states; Application Support storage; backup exclusion + file protection; disk-space check.
  ...
  - **DONE 2026-08-12 (D067).** Actor state machine (missing/downloading/verifying/ready/failed); disk reserve check; injected downloader/capacity seams; incremental verification; staging install; backup exclusion + file protection; cold-launch discovery of valid files; `repair()` for corrupt files; `importPack()` legal-block fallback (identical manifest verification); `resolvedModels()` returning the three production URLs (adapter reads DiT). Synthetic tests in `ModelStoreTests` (CI `31621707347`, 145 tests/0 failures). A real `ModelStore` release download remains correctly blocked by A005/model-assets-v1 not existing.
...
- [x] **K001** — Minimal SwiftUI: model status/download, prompt, seed, Randomize, Generate, Cancel, progress (stage/step/block), elapsed, image, Share, Diagnostics.
  - deps: D006 · output: ContentView/GenerationViewModel · validation: builds; all controls wired
  - **DONE 2026-08-12 (commit `f8448cb` + fixes).** `ContentView` builds the minimal useful app: model section (3 packs with download/import/repair/retry and per-pack missing/downloading/verifying/ready/failed state), prompt (nonempty validation), seed + Randomize (mutates the visible field), Generate (production coordinator, visible seed, resolved 3 URLs), Cancel (cooperative), progress (stage/step/block + elapsed timer), Resume (completed-step count), image display, Share (temp PNG URL via ShareLink), Diagnostics navigation, and user-recoverable errors (no fatalError). `ModelCatalog` binds `ModelStore` states to the UI. CI `31625249377` (151 tests/0 failures). Physical device interaction remains an A12 acceptance item.
- [x] **K002** — GenerationCoordinator: one generation at a time; map/unmap per stage; keep only 512×1024 cond between TE/DiT.
  ...
  - **DONE 2026-08-12 (D064; commit `c8366c3` + fixes).** Reopened by the guide audit and fully resolved. `ResolvedModels` = exactly three production URLs (textEncoder, dit, vae); the DiT pack serves both the adapter and the sampler (no fourth adapter pack). `GenerationEngine` (non-MainActor) runs each stage in its own helper; each heavy model + its `AnimapkFile` mmap is `withExtendedLifetime`-held to the helper boundary and cannot escape — only the fp32 `[512,1024]` conditioning crosses the text-encoder/DiT boundary. The visible seed feeds `SeededRNG(seed:)` (no `UInt64.random` silent override). Metal unavailability is a recoverable `.failed`, not a `fatalError`. Diffusion block progress reaches the UI (`diffusing(step:block:totalSteps:totalBlocks:)`). Cooperative cancel at stage + block boundaries. `GenerationCoordinatorTests` (CI `31609994426`, 116 tests/0 failures): 3-URL topology, adapter==sampler DiT URL, stage order, block progress, per-stage recoverable failure, no-Metal no-crash, single-generation guard, stage-lifetime `deinit` probes. No `/dev/null` paths remain.
...
- [x] **K005** — Diagnostics screen + export JSON + self-test buttons (pack validation, W4/W8 vector, MPS precision, mmap benchmark, GEMM, attention tile, golden-noise self-test).
  - deps: E001–E008, D001–D005, I003 · output: DiagnosticsView + DiagnosticsEngine · validation: runs on device/simulator, exports JSON
  - **DONE 2026-08-12 (commit `f8448cb` + fixes).** `DiagnosticsEngine` builds a Codable structured report (app/OS/device model/Memory/Metal name+availability/disk free/thermal/model-pack status+hashes) and runs deterministic self-tests reusing production primitives: pack validation (ModelManifest size+SHA-256), W4/W8 vector decode (QuantDecoders), MPS fp16 GEMM precision, GEMM (LinearExecutor), attention tile (AttentionExecutor), mmap benchmark, golden-noise RNG (SeededRNG) — each PASS/FAIL/SKIPPED (SKIPPED when Metal genuinely unavailable, never a false PASS). `DiagnosticsView` runs the engine and exports JSON via fileExporter/ShareLink. `DiagnosticsTests` (CI `31625249377`, 151 tests/0 failures) cover Codable round trip, required fields, pass/fail/skip representation, missing models (pack validation FAIL in CI), simulated no-Metal SKIPPED, deterministic RNG, and JSON file creation. Only actual A12 values remain device-only.
...
- [x] **L001** — Full canonical inference integration test: prompt→production Qwen/adapter→golden noise→8-step DiT→VAE at 512/CFG1; compare checkpoints and assert finite.
  ...
  - **DONE 2026-08-12 (commit `3dae37d`; prompt fix `f72a962`; gate `4f84b7a`).** `FullInferenceTests` compiles in normal CI and uses correct production APIs: `QwenEncoderMetal.execute(tokenIDs:output:)`, `LLMAdapterMetal.execute(qwenContext:contextTokens:t5IDs:t5Weights:output:)`, `DiffusionSampler.execute(initialLatent:crossContext:outputLatent:)`, `VAEDecoder.decode(latent:)→DecodedRGBA8`. Production `TokenizerLoader` semantics (Qwen: no specials; T5: no specials + `[1]` EOS; t5Weights all 1.0 verified from fixture JSON). **RGB gap closed (D071):** added canonical `case1_decoded_rgb8.bin` (512×512×3 UInt8, SHA `a396c4ae…7019`) derived from the SHA-verified canonical NPZ `decoded_rgb` via the exact production display transform, so `FullInferenceTests` now actually exercises a full-image regression (previously the fixture was absent and the RGB block never ran). Reports `FULL_RGB_COSINE/RMSE/MAE/MAXABS`, gate cosine ≥ 0.65 (calibrated D074). `scripts/extract_golden_fixtures.py` regenerates the fixture (verified byte-identical). **Prompt bug fixed (D073):** the test used the short prompt while the canonical golden used the long prompt (fixtures.json), so the first full run compared apples-to-oranges (latent 0.488, RGB 0.438). Now uses the exact canonical prompt; calibration run `31638695924` measured latent cosine 0.6946 and RGB cosine 0.7035 (D074). Fixture-gated: skips cleanly when model packs unavailable; `full-inference.yml` injects real packs and requires a PASS.
