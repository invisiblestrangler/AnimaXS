...
Commits: `0234a1e` (feat), `e59af20` (fix tokenStride scope), `5804a8c` (fix attendedToken definite-init), `bb398eb` (test headDim=128 alignment), `4e29342` (test transpose reference to token-major). 
...
- **The last CI run `31910549145` (on `4e29342`) is in-flight.** project-consistency ✓, iphone-build ✓. **Wait for simulator-tests.** If green → P4 gate met, update state files, proceed to P5. If a P4 test fails → investigate.
