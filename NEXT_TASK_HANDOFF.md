...
The runbook-defined refinement cycle is complete on `main`; the refinement
implementation is at `df1ac1b`. The authoritative execution instructions
remain in `RUNBOOK.md`.
...
- Normal CI `31671071004` is green: 153 tests, 13 expected skips, 0 failures.
- Full inference workflow `31671198927` ran both exact canonical candidates on
...
- W8-v2 is the selected candidate and records `full_inference: PASS`: latent
  cosine `0.7522`, RGB cosine `0.7171`, and the cleanest generated image of
  the tested DiT variants. W4-v2 records `full_inference: FAIL`: latent
  cosine `0.6488`, RGB cosine `0.6111`, plus visibly stronger washout and
  texture. The W8 image still has a fine grid texture; that limitation is
  explicit rather than hidden by changing the gate.
